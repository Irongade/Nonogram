...
public class NonogramUI {
	...
	public NonogramUI() {
		...
		try {
			gameFs = new Scanner(new File(NGFILE));
		} catch (FileNotFoundException e) {
			System.out.println(NGFILE + "not found");
		}
		puzzle     = new Nonogram(gameFs);
	}
  ...
  private void move() {
	...
	String singleGameMove = "single " + userMove.getRow() + "," + userMove.getCol() + "," + userMove.getState() + "," + 0;
	gameStack.add(singleGameMove);
  ...
  /**
	...
	private void undo() {
		...
		String lastMoveFromStack = gameStack.pop();
		Assign move = processGameLineAndReturnMove(lastMoveFromStack);

		String moveType = getGameLineType(lastMoveFromStack);

		System.out.println();
		System.out.println(lastMoveFromStack);
		System.out.println(moveType);
		System.out.println();


		if (moveType.equalsIgnoreCase("single")) {
			puzzle.setState(move.getRow(), move.getCol(), 2);
			System.out.println(gameStack);

		} else if(moveType.equalsIgnoreCase("multi")) {
			// remove last move from stack.
			puzzle.setState(move.getRow(), move.getCol(), 2);
			// get np of moves made
			int numOfMovesMade = getNumberOfMultiMovesMade(lastMoveFromStack);

			System.out.println(numOfMovesMade);

			for (int i = 0; i < numOfMovesMade; i++) {
				if (gameStack.isEmpty()) return;

				String lastMultiMoveFromStack = gameStack.pop();
				Assign multiMove = processGameLineAndReturnMove(lastMultiMoveFromStack);
				puzzle.setState(multiMove.getRow(), multiMove.getCol(), 2);
				System.out.println(gameStack);
			}
	...
	private void save() {
		try {
			PrintStream fileScaner = new PrintStream(new File("gameOutput.txt"));

			for (String move : gameStack) {
				fileScaner.println(move);
			}
	...
	private void load() {
		try {
			Scanner fileScaner = new Scanner(new File("gameOutput.txt"));
			puzzle.clear();
			gameStack.clear();
			while(fileScaner.hasNext()) {
				String gameLine = fileScaner.nextLine();

				Assign move = processGameLineAndReturnMove(gameLine);
				String moveType = getGameLineType(gameLine);
				int noOfMultiMovesMade = getNumberOfMultiMovesMade(gameLine);

				if (move != null ) {
					puzzle.setState(move);
					String singleGameMove = moveType + " " + move.getRow() + "," + move.getCol() + "," + move.getState() + "," + noOfMultiMovesMade;
					gameStack.add(singleGameMove);
				}
			}
	...
	private Assign processGameLineAndReturnMove(String gameline) {
		String[] fields = gameline.split(" ");
		Assign move = null;

		if (fields[1] != null && !fields[1].isEmpty()) {
			System.out.println(fields[1]);
			String[] splitMoves = fields[1].split(",");
			try {
				int row = Integer.parseInt(splitMoves[0]);
				int col = Integer.parseInt(splitMoves[1]);
				int state = Integer.parseInt(splitMoves[2]);

				System.out.println(row);
				System.out.println(col);
				System.out.println(state);

				move = new Assign(row, col, state);

			} catch(NumberFormatException e) {
				System.out.println("could not parse moves");
			}
		}
	...
	private String getGameLineType(String gameline) {
		String[] fields = gameline.split(" ");
		return fields[0];
	}

	private int getNumberOfMultiMovesMade(String gameline) {
		String[] fields = gameline.split(" ");

		String[] splitMoves = fields[1].split(",");


		return Integer.parseInt(splitMoves[3]);
	}
  ...
  private void rowMultiMove() {
    ...
    for (Assign a : list) {
    	puzzle.setState(a);
		String singleGameMove = "multi " + a.getRow() + "," + a.getCol() + "," + a.getState() + "," + index;
		gameStack.add(singleGameMove);
		index++;
	}
   ...
   */
	...
	private Stack<String> 	gameStack = null;
	// creating a global scanner variable to hold the game settings
	Scanner gameFs = null;
